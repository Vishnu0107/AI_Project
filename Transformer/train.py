...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # Precompute the encoder output and reuse it for every we get from the decoder
    ...
    while True:
        ...
        # Build the decoder input with the sos input
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_examples = 2):
    model.eval() # Tells pytorch to run the model in evaluation mode
    # We inference two sentences and check the output of the model
    count = 0
    '''
    source_texts = []
    expected = [] 
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            # We use create it once and reuse it for every sentence so we will be creating a greedy decode function
...
def train_model(config):
    ...
    # Tensorboard -> this is to visualize the training
    ...
    # Decreases its confidence in the predictions of the model
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            decoder_mask = batch['decoder_mask'].to(device) # (0, 1, 1, seq_len)
